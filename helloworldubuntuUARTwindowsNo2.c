	...
	while(1)
	{
		...
		if ((x % 5) ==0)
		{
			...
			if(uart_r_len > 0)//uart_r_len有寫入
			{
				...
				if (m_kk.command[0] == 'A' && m_kk.command[1] == '2')//新增判斷m_kk.command[0] 內容為 'A'且m_kk.command[1] 內容為 '2'
				{//新增
					printf("收到主控A2\r\n");//新增指示有收到相同信號
					Flag_get_CND_oxA2_reboot_ins=1;//旗標Flag_get_CND_oxA2_reboot_ins設1指有收到A2
				}//新增
				if (m_kk.command[0] == 'A' && m_kk.command[1] == '3')//新增判斷m_kk.command[0] 內容為 'A'且m_kk.command[1] 內容為 '3'
				{//新增
					printf("收到主控A3\r\n");//新增指示有收到相同信號
				...
				if (m_kk.command[0] == 'A' && m_kk.command[1] == '4')//新增判斷m_kk.command[0] 內容為 'A'且m_kk.command[1] 內容為 '3'
				{//新增
					printf("收到主控A4\r\n");//新增指示有收到相同信號

					//buffertemp[43]=m_kk.P1[0];
					//sprintf(buffertemp,"mkdir /home/yu/example/20201110_practice/A4%s",&m_kk.P1[0]);
